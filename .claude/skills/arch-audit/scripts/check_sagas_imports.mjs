...
function extractSagasImports(content) {
  const imports = [];
...
function checkSagasImports() {
  console.log('Sagas Import Check (Composition Root Pattern)');
  console.log('='.repeat(80));
  console.log('');
  console.log('Rule: Feature sagas.ts files MUST ONLY be imported in:');
  ...
  for (const file of allFiles) {
    ...
    const imports = extractSagasImports(content);
  ...
  if (violations.length === 0) {
    ...
    console.log('');
    console.log(`All sagas imports are properly isolated in ${ALLOWED_FILE}`);
  } else {
    ...
    for (const [file, viols] of Object.entries(byFile)) {
      ...
      console.log(`     Fix: Remove this import. Sagas are only imported in ${ALLOWED_FILE}`);
      console.log('');
    }
  }
...
const exitCode = checkSagasImports();
